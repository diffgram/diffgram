...
@routes.route('/api/v1/project/<string:project_string_id>/roles/<int:role_id>/remove-perm', methods = ['PATCH'])
@Project_permissions.user_has_project(Roles = ["admin"], apis_user_list = ["api_enabled_builder"])
def role_remove_permission_web(project_string_id, role_id):
    """
    ...
    with sessionMaker.session_scope() as session:
        ...
        role_data, log = role_remove_permission_core(
            session = session,
            log = log,
            member = member,
            project = project,
...
def role_remove_permission_core(session,
    ...
    role_with_new_perm, log = role.remove_permission(
        session = session,
        perm = permission,
