...
def job_cancel_api():
	...
	with sessionMaker.session_scope() as session:
		...
		"""
		May be 1 job or a list of jobs, so we build a list either way
		so job_cancel_core and similar code is reused

		The keys are guaranteed to be there from regular input,
		but the values may be None.

		We assume that the project roles will be checked
		by job_cancel_core
