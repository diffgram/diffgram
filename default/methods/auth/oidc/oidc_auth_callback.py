...
def login_and_return_access_token(session, diffgram_user, user_data, access_token_data, log):
    """
    Log in the user and return the access token.
    """
...
def api_oidc_callback():
    """
    OIDC Callback endpoint for handling OAuth2 callback.
    """
    oidc_spec_list = [
        {"code": {
            'kind': str
    ...
    with sessionMaker.session_scope() as session:
        ...
        diffgram_user = User.get_user_by_oauth2_id(session=session, oidc_id=user_id)
