...
class TestTaskEvent(testing_setup.DiffgramBaseTestCase):
    ...
    def test_get_task_from_job_id(self):
        ...
        task1 = data_mocking.create_task({'name': 'test task', 'file': file, 'job': job, 'status': 'available'}, self.session)
        task2 = data_mocking.create_task({'name': 'test task', 'file': file, 'job': job, 'status': 'available'}, self.session)
