			...
			for instance in instance_list:
				...
				if 'sequence_id' not in instance:
					...
					if instance['sequence_id'] is None:
						self.input.update_log['error'][str(frame_number)] = "[Sequence Match Error] At frame " + str(frame_number) + \
							" the instance with label_file_id: " + str(label_file_id) + " the number " + str(number) + \
							" does not match an available sequence. " + " available pairs are " + str(self.sequence_number_to_id)
	...
	def push_frames_for_copy_to_queue(
		...
		for frame in source_video_frames:
			...
			frame_input = Input.new(
				...
				parent_file_id = destination_video_parent_file_id, # This is the parent video file where all data is going to be copied.
	...
	def create_sequences_not_yet_existing(
		...
		if label_file is None:
			self.input.update_log['error'][str(label_file_id)] = "Label File ID: " + str(label_file_id) + \
				"Does not exist in the project. Found at sequence Number: " + str(number)
			return
