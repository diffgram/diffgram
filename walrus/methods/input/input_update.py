...
class Update_Input():
	...
	def build_input_list_eligible_for_retry(self):
		...
		if self.input_list and settings.DIFFGRAM_SYSTEM_MODE == 'production':
			try:
				communicate_via_email.send(
					email = "@.com",
					subject = str(len(self.input_list)) + " retried [Auto Retry].",
					message = "First project in list: " + \
					...
					"\n prior percent complete: " + ", ".join([str(i.percent_complete) for i in self.input_list]) + \
