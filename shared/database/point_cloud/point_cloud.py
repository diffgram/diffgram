...
class PointCloud(Base):
    ...
    def regenerate_url(self, session):
        if session and self.url_signed_blob_path:
            ...
            print(self.url_signed_expiry, 'aaaaa', time_to_check)
            url_signed_expiry = float(self.url_signed_expiry)
            if url_signed_expiry is None or url_signed_expiry <= time_to_check:
