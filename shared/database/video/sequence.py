...
class Sequence(Base):
	...
	def serialize_for_label_subset(self, session = None):
		...
		return {
			...
			'single_frame': self.single_frame,
			'label_file_id': self.label_file_id,
			'instance_preview': self.build_instance_preview_dict(
				session = session)
		}
	...
	def get_preview_instance(self, session):
		...
		instance = session.query(Instance).filter(
			...
			Instance.soft_delete == False).order_by(Instance.id).first()
	...
	def new(number, 
		...
		if label_file:
			...
			logger.info("label_file is None, this is unusual. video_file_id was: " + str(video_file_id) + " number was " + str(number))
	...
	def update_frame_number_list(self, session, instance):
		...
		if instance.soft_delete is False or instance.soft_delete is None:
			...
			try: del frame_number_list[bisect.bisect_left(
