...
export default Vue.extend( {
  ...
  computed: {
    avatarName: function(){
      ...
      if (!this.user_local || !this.user_local.first_name || !this.user_local.last_name) return 'DG';
    ...
    >
      <v-tooltip bottom>
        <template #activator="{on}">
          <!-- 1.1 Expected case -->
          <div
            v-if="user_local.profile_image_thumb_url"
          >
            <v-avatar
              :size="size"
              v-on="on"
          ...
          >
            <v-avatar
              ...
              v-on="on"
            >
              <span class="white--text headline" :style="{['font-size']: fontSize}">
                {{ avatarName }}
              </span>
            </v-avatar>
