...
export default Vue.extend({
    ...
    props: {
      ...
      'enable_hotkeys': {default: false, type: Boolean},
      'schema_id': {
        default: undefined
      },
    ...
    methods: {
      change_selected_item: function (direction: string = 'next') {
        let current_index = this.label_list.indexOf(this.selected)

        if (direction === 'next') {
          if (current_index === this.label_list.length - 1) {
            this.selected = this.label_list[0]
          } else {
            this.selected = this.label_list[current_index + 1]
          }
          this.emit_selected()
        } else {
          if (current_index === 0) {
            this.selected = this.label_list[this.label_list.length - 1]
          } else {
            this.selected = this.label_list[current_index - 1]
          }
          this.emit_selected()
      ...
      manage_key_down: function () {
        if (event.keyCode === 40) {  // Arrow Down
          this.change_selected_item('next')


        } else if (event.keyCode === 38) {  // Arrow UP
          this.change_selected_item('previous')
      ...
      remove_hotkeys: function(){
        window.removeEventListener(
          "keyup",
          this.manage_key_down
        );
      },
      add_hotkeys: function () {
