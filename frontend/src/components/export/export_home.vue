      ...
      <v-container>
        ...
        <v-flex>
          ...
          <v-alert type="info"
            ...
            <v-chip color="red"
                    text-color="white"
                    >{{active_instances}} </v-chip> active instances.

            Free Plan has limit of <v-chip color="white">100</v-chip>.
      ...
      axios.post( '/api/walrus/project/' + String(this.project_string_id)
        ...
        .then(response => {
          ...
          if (e.response.data.log &&
            e.response.data.log.error.over_free_plan_limit == true) {
            this.over_free_plan_limit = true
            this.active_instances = e.response.data.log.error.active_instances
          }
