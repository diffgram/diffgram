...
export default Vue.extend({
  ...
  methods: {
    ...
    async add_output_actions_to_job() {
      ...
      this.error_launch = {};
      if (
        this.output_dir.action === "copy" ||
        this.output_dir.action === "move"
      ) {
        if (!this.output_dir.directory) {
          this.error_launch = {
            output_dir:
              "Please select a directory for copy/move after tasks are completed.",
          };
          this.loading = false;
          return false;
        }
      ...
      try {
        const response = await axios.post(
          ...
          {
            output_dir: this.output_dir.directory
              ? this.output_dir.directory.directory_id.toString()
              : undefined,
