            ...
            <v-toolbar-items>
              ...
              <div v-else class="pl-2 pt-4">
                ...
                >{{ start_index_oneth_index }} to
                  {{ metadata_previous.end_index }}
                </v-chip>
                of
                <v-chip class="pl-2 pr-2"
                        color="white"
                        text-color="primary"
                >{{ metadata_previous.file_count }}
                </v-chip>
                ...
                >
                  <a @click="change_file_request(item)" style="position: relative">
                    <div v-if="current_file && item.id == current_file.id"
                         style="position: absolute; top: -10px; right: -18px; z-index: 99999">
                      <tooltip_icon
...
export default Vue.extend({
    ...
    mounted() {
      ...
      var self = this
      this.media_refresh_watcher = this.$store.watch((state) => {
          return this.$store.state.media.refresh
        },
        (new_val, old_val) => {
          self.request_media()
        },
