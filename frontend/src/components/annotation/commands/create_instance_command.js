export class CreateInstanceCommand {
  ...
  execute() {
    if (this.instance.id) {
      ...
      const existing_instance = this.ann_core_ctx.instance_list.filter(instance => instance.creation_ref_id === this.instance.creation_ref_id)
      if (existing_instance.length === 0) {
        ...
        if(this.instance.type === 'polygon' && !polygon_alert_shown){
          ...
          this.ann_core_ctx.$store.commit('set_user_setting', ['polygon_autoborder_info', true])
        }
      } else {
        const existing_instance = this.ann_core_ctx.instance_list.filter(instance => instance.creation_ref_id === this.instance.creation_ref_id)[0]
  ...
  undo() {
    const instance = this.ann_core_ctx.instance_list[this.created_instance_index];

    // We don't want to delete instances that already have an ID on backend, just soft delete them.
    instance.soft_delete = true;
    this.ann_core_ctx.instance_list.splice(this.created_instance_index, 1, instance);
