...
export default class DeleteInstanceCommand extends Command {
    ...
    private find_relations(instance: Instance) {
        ...
        this.instance_list.get_all()
            .map((inst, index) => {
                ...
                if (inst.type === "relation" && (id === from_instance_id || id === to_instance_id)) this.deleted_relations.push(index)
            })
    }
