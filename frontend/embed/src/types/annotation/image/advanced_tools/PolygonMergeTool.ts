...
export class PolygonMergeTool {
  ...
  public delete_instances_and_add_to_merged_instance(parent_instance: Instance,
    ...
    for (const instance of instances_to_merge) {
      ...
      for (const point of instance.points) {
        let currPoint = point as PolygonPoint
        let new_figure_id = figure_id;
        if (currPoint.figure_id) {
          new_figure_id = currPoint.figure_id;
        }
        new_points.push({
          ...currPoint,
  ...
  public merge_polygons(instance_list: Instance[]): number[]{
    ...
    let parent_points = parent_instance.points as PolygonPoint[]
    let has_multiple_figures = parent_points.filter((p) => p.figure_id != undefined).length > 0;
    let deleted_instance_indexes = []
